...
func New_cpu_intensive_processes(Pid int32, User string, Cpu float64, Rss uint64, Vsz uint64, Cmd string) *cpu_intensive_processes {
...
func get_cpu_intensive_processes() *cpu_intensive_processesSlice {
	...
	for i := range members[:15] {
		...
		s_cpu_intensive_processesSlice = append(s_cpu_intensive_processesSlice, cpu_intensive_processes{members[i].Id, user, cpu, rss, vms, cmd})
	}
